...
export default function Home() {
  ...
  return (
    <div>
      <div className=" text-4xl bg-lime-500 w-fit mx-auto p-2 rounded-full">
        NextJS Todo CRUD
      </div>
      ...
      <div className="grid grid-cols-1  md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-4 ">
        {todos.map((task) => (
          <div
            key={task._id}
            className="w-full p-6 border border-gray-400 rounded-lg shadow-xl "
          >
            <h1
              className={`text-3xl mb-2 font-semibold ${task.completed ? "line-through" : ""
                }`}
            >
              {task.title}
            </h1>
